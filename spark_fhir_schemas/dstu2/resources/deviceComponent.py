...
class DeviceComponentSchema:
    ...
    processing a resource are required to check for modifier extensions.
        type: Describes the specific component type as defined in the object-oriented or
    metric nomenclature partition.
        identifier: Describes the local assigned unique identification by the software. For
    example: handle ID.
        lastSystemChange: Describes the timestamp for the most recent system change which includes
    device configuration or setting change.
        source: Describes the link to the source Device that contains administrative device
    information such as manufacture, serial number, etc.
        parent: Describes the link to the parent resource. For example: Channel is linked to
    its VMD parent.
        operationalStatus: Indicates current operational status of the device. For example: On, Off,
    Standby, etc.
        parameterGroup: Describes the parameter group supported by the current device component that
    is based on some nomenclature, e.g. cardiovascular.
        measurementPrinciple: Describes the physical principle of the measurement. For example: thermal,
    chemical, acoustical, etc.
        productionSpecification: Describes the production specification such as component revision, serial
    number, etc.
    ...
    device. This language code will follow the IETF language tag. Example: en-US.
        ...
        schema = StructType(
            [
                ...
                # device configuration or setting change.
                StructField("lastSystemChange", ElementSchema.get_schema(max_nesting_depth=max_nesting_depth,nesting_depth=nesting_depth+1,nesting_list=my_nesting_list,max_recursion_limit=max_recursion_limit,include_extension=include_extension,extension_fields=extension_fields, extension_depth=extension_depth+1, max_extension_depth=max_extension_depth), True),
                ...
                # Standby, etc.
                StructField("operationalStatus", CodeableConceptSchema.get_schema(max_nesting_depth=max_nesting_depth,nesting_depth=nesting_depth+1,nesting_list=my_nesting_list,max_recursion_limit=max_recursion_limit,include_extension=include_extension,extension_fields=extension_fields, extension_depth=extension_depth+1, max_extension_depth=max_extension_depth), True),
