...
class Conformance.OperationSchema:
    """
    ...
    processing a resource are required to check for modifier extensions.
        name: The name of a query, which is used in the _query parameter when the query is
    called.
        ...
        if (max_recursion_limit and nesting_list.count("ConformanceOperation") >= max_recursion_limit) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list+["ConformanceOperation"]
        schema = StructType(
            [
                # None
                ...
                StructField("definition", ReferenceSchema.get_schema(max_nesting_depth=max_nesting_depth,nesting_depth=nesting_depth+1,nesting_list=my_nesting_list,max_recursion_limit=max_recursion_limit,include_extension=include_extension,extension_fields=extension_fields, extension_depth=extension_depth+1, max_extension_depth=max_extension_depth), True),
            ]
        )
