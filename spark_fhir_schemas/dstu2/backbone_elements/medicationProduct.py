...
class MedicationProductSchema:
    ...
    production run.
        ...
        # ingredient
        from spark_fhir_schemas.dstu2.backbone_elements.medicationingredient import MedicationIngredientSchema
        # batch
        from spark_fhir_schemas.dstu2.backbone_elements.medicationbatch import MedicationBatchSchema
        if (max_recursion_limit and nesting_list.count("MedicationProduct") >= max_recursion_limit) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list+["MedicationProduct"]
        schema = StructType(
            [
                # None
                ...
                StructField("form", CodeableConceptSchema.get_schema(max_nesting_depth=max_nesting_depth,nesting_depth=nesting_depth+1,nesting_list=my_nesting_list,max_recursion_limit=max_recursion_limit,include_extension=include_extension,extension_fields=extension_fields, extension_depth=extension_depth+1, max_extension_depth=max_extension_depth), True),
                # Identifies a particular constituent of interest in the product.
                StructField("ingredient", MedicationIngredientSchema.get_schema(max_nesting_depth=max_nesting_depth,nesting_depth=nesting_depth+1,nesting_list=my_nesting_list,max_recursion_limit=max_recursion_limit,include_extension=include_extension,extension_fields=extension_fields, extension_depth=extension_depth+1, max_extension_depth=max_extension_depth), True),
                # Information about a group of medication produced or packaged from one
                # production run.
                StructField("batch", MedicationBatchSchema.get_schema(max_nesting_depth=max_nesting_depth,nesting_depth=nesting_depth+1,nesting_list=my_nesting_list,max_recursion_limit=max_recursion_limit,include_extension=include_extension,extension_fields=extension_fields, extension_depth=extension_depth+1, max_extension_depth=max_extension_depth), True),
            ]
        )
