...
class Evidence_VariableSchema:
    ...
    def get_schema(
        ...
        if (
            max_recursion_limit
            and nesting_list.count("Evidence_Variable") >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["Evidence_Variable"]
        my_parent_path = (
            parent_path + ".evidence_variable" if parent_path else "evidence_variable"
        )
        schema = StructType(
            [
