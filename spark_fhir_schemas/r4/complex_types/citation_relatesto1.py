...
class Citation_RelatesTo1Schema:
    """
    ...
    def get_schema(
        ...
        if (
            max_recursion_limit
            and nesting_list.count("Citation_RelatesTo1") >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["Citation_RelatesTo1"]
        my_parent_path = (
            parent_path + ".citation_relatesto1"
            if parent_path
            else "citation_relatesto1"
        )
        schema = StructType(
            [
