...
class Evidence_VariableDefinitionSchema:
    """
    ...
    def get_schema(
        ...
        variableRole: population | subpopulation | exposure | referenceExposure | measuredVariable |
            confounder.
        ...
        if (
            max_recursion_limit
            and nesting_list.count("Evidence_VariableDefinition") >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["Evidence_VariableDefinition"]
        my_parent_path = (
            parent_path + ".evidence_variabledefinition"
            if parent_path
            else "evidence_variabledefinition"
        )
        schema = StructType(
            [
