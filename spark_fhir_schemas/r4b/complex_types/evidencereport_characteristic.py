...
class EvidenceReport_CharacteristicSchema:
    """
    ...
    def get_schema(
        ...
        if (
            max_recursion_limit
            and nesting_list.count("EvidenceReport_Characteristic")
            >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["EvidenceReport_Characteristic"]
        my_parent_path = (
            parent_path + ".evidencereport_characteristic"
            if parent_path
            else "evidencereport_characteristic"
        )
        schema = StructType(
            [
