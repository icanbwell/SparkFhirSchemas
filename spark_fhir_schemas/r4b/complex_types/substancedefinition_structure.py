...
class SubstanceDefinition_StructureSchema:
    """
    ...
    def get_schema(
        ...
        from spark_fhir_schemas.r4b.complex_types.substancedefinition_representation import (
            SubstanceDefinition_RepresentationSchema,
        )

        if (
            max_recursion_limit
            and nesting_list.count("SubstanceDefinition_Structure")
            >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["SubstanceDefinition_Structure"]
        my_parent_path = (
            parent_path + ".substancedefinition_structure"
            if parent_path
            else "substancedefinition_structure"
        )
        schema = StructType(
            [
                ...
                StructField(
                    "representation",
                    ArrayType(
                        SubstanceDefinition_RepresentationSchema.get_schema(
                            max_nesting_depth=max_nesting_depth,
                            nesting_depth=nesting_depth + 1,
                            nesting_list=my_nesting_list,
                            max_recursion_limit=max_recursion_limit,
