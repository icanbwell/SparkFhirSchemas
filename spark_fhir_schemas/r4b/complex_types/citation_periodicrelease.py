...
class Citation_PeriodicReleaseSchema:
    """
    ...
    def get_schema(
        ...
        from spark_fhir_schemas.r4b.complex_types.citation_dateofpublication import (
            Citation_DateOfPublicationSchema,
        )

        if (
            max_recursion_limit
            and nesting_list.count("Citation_PeriodicRelease") >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["Citation_PeriodicRelease"]
        my_parent_path = (
            parent_path + ".citation_periodicrelease"
            if parent_path
            else "citation_periodicrelease"
        )
        schema = StructType(
            [
                ...
                StructField(
                    "dateOfPublication",
                    Citation_DateOfPublicationSchema.get_schema(
                        max_nesting_depth=max_nesting_depth,
                        nesting_depth=nesting_depth + 1,
                        nesting_list=my_nesting_list,
                        max_recursion_limit=max_recursion_limit,
