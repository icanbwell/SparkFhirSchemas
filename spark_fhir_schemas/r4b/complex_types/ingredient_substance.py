...
class Ingredient_SubstanceSchema:
    """
    ...
    def get_schema(
        ...
        from spark_fhir_schemas.r4b.complex_types.ingredient_strength import (
            Ingredient_StrengthSchema,
        )

        if (
            max_recursion_limit
            and nesting_list.count("Ingredient_Substance") >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["Ingredient_Substance"]
        my_parent_path = (
            parent_path + ".ingredient_substance"
            if parent_path
            else "ingredient_substance"
        )
        schema = StructType(
            [
                ...
                StructField(
                    "strength",
                    ArrayType(
                        Ingredient_StrengthSchema.get_schema(
                            max_nesting_depth=max_nesting_depth,
                            nesting_depth=nesting_depth + 1,
                            nesting_list=my_nesting_list,
                            max_recursion_limit=max_recursion_limit,
