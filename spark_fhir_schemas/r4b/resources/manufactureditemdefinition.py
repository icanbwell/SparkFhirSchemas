...
class ManufacturedItemDefinitionSchema:
    """
    ...
    def get_schema(
        ...
        from spark_fhir_schemas.r4b.complex_types.manufactureditemdefinition_property import (
            ManufacturedItemDefinition_PropertySchema,
        )

        if (
            max_recursion_limit
            and nesting_list.count("ManufacturedItemDefinition") >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["ManufacturedItemDefinition"]
        my_parent_path = (
            parent_path + ".manufactureditemdefinition"
            if parent_path
            else "manufactureditemdefinition"
        )
        schema = StructType(
            [
                ...
                StructField(
                    "property",
                    ArrayType(
                        ManufacturedItemDefinition_PropertySchema.get_schema(
                            max_nesting_depth=max_nesting_depth,
                            nesting_depth=nesting_depth + 1,
                            nesting_list=my_nesting_list,
                            max_recursion_limit=max_recursion_limit,
