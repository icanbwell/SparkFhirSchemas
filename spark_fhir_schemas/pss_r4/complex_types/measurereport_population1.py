...
class AutoMapperElasticSearchMeasureReport_Population1(AutoMapperDataTypeComplexBase):
    """
    ...
    def __init__(
        ...
        subjectResults: Optional[Any] = None,
    ) -> None:
        super().__init__(
            id_=id_,
            extension=extension,
    ...
    def schema(
        ...
        if (
            max_recursion_limit
            and nesting_list.count("MeasureReport_Population1") >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["MeasureReport_Population1"]
        schema = StructType(
            [
