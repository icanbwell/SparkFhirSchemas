...
class AutoMapperElasticSearchMolecularSequence_Inner(AutoMapperDataTypeComplexBase):
    """
    ...
    def schema(
        ...
        start: Structural variant inner start. If the coordinate system is either 0-based or
            1-based, then start position is inclusive.

        end: Structural variant inner end. If the coordinate system is 0-based then end is
        ...
        if (
            max_recursion_limit
            and nesting_list.count("MolecularSequence_Inner") >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["MolecularSequence_Inner"]
        schema = StructType(
            [
