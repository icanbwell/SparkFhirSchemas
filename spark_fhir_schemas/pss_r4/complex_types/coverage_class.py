...
class AutoMapperElasticSearchCoverage_Class(AutoMapperDataTypeComplexBase):
    """
    ...
    def schema(
        ...
        type: The type of classification for which an insurer-specific class label or number
        ...
        if (
            max_recursion_limit
            and nesting_list.count("Coverage_Class") >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["Coverage_Class"]
        schema = StructType(
            [
