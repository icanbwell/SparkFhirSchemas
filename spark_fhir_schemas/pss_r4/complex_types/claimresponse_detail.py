...
class AutoMapperElasticSearchClaimResponse_Detail(AutoMapperDataTypeComplexBase):
    """
    ...
    def schema(
        ...
        from spark_fhir_schemas.pss_r4.complex_types.claimresponse_subdetail import (
            AutoMapperElasticSearchClaimResponse_SubDetail as ClaimResponse_SubDetailSchema,
        )

        if (
            max_recursion_limit
            and nesting_list.count("ClaimResponse_Detail") >= max_recursion_limit
        ) or (max_nesting_depth and nesting_depth >= max_nesting_depth):
            return StructType([StructField("id", StringType(), True)])
        # add my name to recursion list for later
        my_nesting_list: List[str] = nesting_list + ["ClaimResponse_Detail"]
        schema = StructType(
            [
                ...
                StructField(
                    "subDetail",
                    ArrayType(
                        ClaimResponse_SubDetailSchema.schema(
                            max_nesting_depth=max_nesting_depth,
                            nesting_depth=nesting_depth + 1,
                            nesting_list=my_nesting_list,
                            max_recursion_limit=max_recursion_limit,
