...
def format_property(p: str) -> str:
    if keyword.iskeyword(p) or p in dir(builtins):
        return f"{p}_"
    else:
        return p
...
def main() -> int:
    ...
    # clean out old stuff
    (
        complex_types_folder,
        resources_folder,
        simple_types_folder,
    ) = clean_up_old_materials(data_dir)
...
def clean_up_old_materials(data_dir):
    ...
    return complex_types_folder, resources_folder, simple_types_folder
